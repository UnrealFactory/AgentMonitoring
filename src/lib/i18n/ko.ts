...
export const ko: Dict = {
  ...
  /* 한글 이름으로는 슬러그가 만들어지지 않는다 — 슬러그는 디렉터리 이름이고 CLI도 `결제` 같은
     id는 `invalid id`로 거절한다. 규칙이 맞으니 고칠 것은 말이다: 왜 안 되는지, 그리고 지금 무엇을
     하면 되는지를 이 한 줄이 다 말한다. 만들기 버튼이 눌리지 않는 동안 화면에 있는 문장은 이것
     하나뿐이기 때문이다(P9 7·8라운드 크리틱). 조사 `만`은 앞말에 붙여 쓴다. */
  "proj.form.slugNeeded":
    "이 이름으로는 슬러그를 만들 수 없습니다. 슬러그는 projects/ 아래의 디렉터리 이름이라 소문자와 숫자, -, _만 쓸 수 있습니다. 여기에 직접 입력해 주세요.",
