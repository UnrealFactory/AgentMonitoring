...
a feature list. The CLI's sixteen commands are consolidated into **five tools shaped like
the workflow** (do work, update work, file a bug, fix a bug, look around) instead of
mirrored one-for-one; each description is one verb-first sentence and no property
description repeats what its name already says; the whole `tools/list` response, schemas
included, is **4,650 bytes**. Results are the other half: a write returns the id, the file
path and one line of confirmation — the CLI's `--json` envelope boiled down, about 200
characters, rather than the record it just wrote, which the caller already has. Reads
return compact summaries capped at 600 characters, and `status(mode="view", full=true)` is
the single, explicit way to pay for a whole record. `npm test` measures all of it and fails
if any budget is exceeded, so the numbers in this paragraph cannot quietly rot.

## Install

Build the CLI first — the server shells to it:
...
Then add the server, filling in your vault, project and agent handle:

```bash
claude mcp add agentmon -- node C:/Code/AgentMonitoring/mcp/server.mjs --vault C:/AgentVault --project myproj --agent my-agent
```

That is the whole configuration. **Identity lives in the `mcp add` line, not in every
call**: the vault the records go to, the default project and the agent handle written onto
them are given once at startup, which is why no tool call has to spend tokens repeating
them. Any call may still override the project or the agent (a second project in the same
vault, or a subagent with its own handle) by passing `project` or `agent`.

For this repository's own history:

```bash
claude mcp add agentmon -- node C:/Code/AgentMonitoring/mcp/server.mjs --vault C:/Code/AgentMonitoring/vault --project agent-monitoring --agent my-agent
```
...
| Flag | |
|---|---|
| `--vault <dir>` | **Required.** The vault to read and write. |
| `--project <slug>` | Default project for every call; a call may override it. |
| `--agent <handle>` | Default agent handle written onto records; a call may override it. |
| `--bin <path>` | The agentmon binary. Else `$AGENTMON_BIN`, else `target/release`, else `PATH`. |

`--vault` has no default **on purpose**. The CLI falls back to `./vault` when it finds one,
which is right for a human in a repository and wrong for an MCP server: its working
directory is whatever the client happened to launch it in, so a fallback would decide the
destination of a real write by accident. With no `--vault` the server refuses to start
(exit 2) and says so. Any `AGENTMON_VAULT`, `AGENTMON_PROJECT` or `AGENTMON_AGENT` in the
environment is stripped from the CLI child for the same reason: the startup flags are the
only identity.

## The tools

`*` marks a required argument. `project` and `agent` are optional on every tool and
...
|---|---|---|
| `0` | Success | — |
| `1` | Vault has problems | `doctor`-level errors in the vault |
| `2` | Usage or timestamp rejected | A time in the future, or one before the state it follows |
| `3` | Not found | Unknown project slug, or an id that does not exist |
...
| `6` | Invalid vault | Corrupt frontmatter or `vault.json` |
| `7` | I/O error | Permissions, or the path is gone |

Two failures come from the server rather than the CLI, and say so plainly: no project or
agent available for the call, and `update_work` with nothing to say. A failed call writes
...
vault in the temp directory: handshake, `tools/list`, then a full work lifecycle (open,
note, close, correct, and a backdated one-call record) and a full bug lifecycle (file,
comment without claiming, then claim + comment + resolve), every read mode, the error
paths, and `agentmon doctor --strict` over the result. The budgets are asserted on the
bytes that actually cross stdio, not on a re-serialized object. Two guards matter: the test
launches a server from the repository root with no `--vault` and asserts it refuses rather
than finding `./vault`, and it fingerprints the repository's live vault before and after to
