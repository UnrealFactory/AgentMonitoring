...
**The same release stopped a rewrite passing as a first draft.** Before rewriting a note by name, that server asks whether it exists. Any failure of that question used to read as "there is no such note". Only the answer that really means "there is none" counts now.
