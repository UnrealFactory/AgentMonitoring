...
updated: 2026-08-24T04:09:42Z
tags: [human-area, pictures, gates]
refs: [WORK-0078, BUG-0025]
---

The human area's pictures are **per-beat scenes** (`docs/HUMAN_STYLE.md`, "A picture is one
beat's scene"): one drawing per beat that earns one, drawn inside the beat above its words.
The contract names `npm run check:scenes` as what settles the bands and the type floor; this
note is the operating detail it does not carry — how to run it, and what else it knows.
...
honesty law is per element, and the trace is the only place the reasoning survives.

**The measured column.** Walking the app's window from 700 to 1920 with a record's Human
view open, the picture column is narrowest at window 1104: **424 CSS px**, where a 22-unit
label lands at 12.5px. The contract tells you to size for 395, which is the conservative
number and the one to draw to; the gate checks against it, with `.prose-img`'s 560px
`max-height` in the arithmetic so a tall scene is measured at the height that actually ships.

## For humans

On 24 August 2026 the drawings in these plain-language pages changed shape: one small
picture per step of a story, sitting above that step's words. This note says how such a
drawing is checked before it ships.
...
wider lettering, standing in for the machines this one is not.
...
**Too small for what, exactly, was measured.** Somebody walked the app's window from 700
pixels wide to 1920 and watched the column a drawing is given. It is narrowest at a window
of 1104, where the drawing gets 424 pixels, and the rules tell you to draw for 395 — the
safer of the two.
