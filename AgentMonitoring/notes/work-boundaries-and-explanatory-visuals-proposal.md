...
refs: [WORK-0107, WORK-0108, WORK-0110]
---
...
이 정책은 현재 소스·개발용 CLI와 2026-09-06 공개한 **v1.5.2**에 포함됐습니다(WORK-0110). 이 저장소의 AGENTS.md를 갱신하고 CLAUDE.md에는 기존 프로젝트 규칙을 보존하여 관리 구간을 추가했습니다. 이 기기의 설치 프로그램 실행과 다른 프로젝트 지침 일괄 갱신은 하지 않았습니다. 앱 업데이트 뒤 각 프로젝트에서 지침 쓰기 → Codex / Claude로 적용하세요. 기기별 `.codex/config.toml`은 변경하거나 커밋하지 않았습니다.
...
시험 데이터와 레지스트리는 모두 OS 임시 폴더로 격리했습니다. baseline은 WORK-0108 시작 당시 HEAD(87c00b3)의 기존 지침, updated는 수정 지침을 읽은 별도 새 에이전트입니다. resume 에이전트에는 이전 대화를 주지 않고 프로젝트 경로·현재 지침·작업 재개 요청만 제공했습니다.
...
- WORK-0108에서는 실제 WebView 앱 설치·공개 배포·실제 Codex 클라이언트 재연결을 포함하지 않았습니다. 공개 배포는 후속 WORK-0110에서 v1.5.2로 완료했습니다. 개별 UI 참조 경로는 공통 함수를 통한 회귀 검증이며 새 링크 클릭의 실앱 검증은 하지 않았습니다.

v1.5.2의 배포 결과와 설치 후 지침 적용 방법은 handoff-v1-release를 읽으세요. 재발한 사례가 생기면 대화 흐름·실제 기록 선택을 이 시험과 비교하시면 됩니다.
...
그림도 문장 상자를 반복하는 대신 대상과 연결 상태를 보여주도록 안내합니다. 위 그림은 새 에이전트가 만든 시험 결과이며 실제 게임 실행 화면은 아닙니다. 코드와 그림 검사는 통과했지만 이 사례만으로 모든 대화의 성공을 보장하지는 않습니다. 저장소 지침과 개발용 CLI에 반영했고, 후속 배포 작업에서 1.5.2로 공개했습니다. 앱 업데이트 뒤 프로젝트별 지침 쓰기로 기존 파일을 갱신할 수 있습니다.
