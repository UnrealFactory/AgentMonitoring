...
type: essential
description: "공개 버전은 v1.7.0입니다(2026-09-13, 지침·MCP 통합 버튼·.claude/CLAUDE.md·우클릭 단일화·로그인 시 자동 실행). 배포는 handoff-v1-release, 자동 실행은 autostart-at-login, 프로젝트·기본값은 project-organization-and-create-defaults를 읽으세요."
agent: fable-updater-splash
updated_by: fable-unified-scaffold
created: 2026-08-21T13:21:48Z
updated: 2026-09-13T11:47:32Z
tags: []
...
- **공개 버전은 v1.7.0입니다(2026-09-13).** [[handoff-v1-release]]에 소스·태그·설치 파일과 공개 다운로드 검증, 다음 배포 절차를 정리했습니다. v1.7.0은 WORK-0122(지침·MCP 통합 버튼, `.claude/CLAUDE.md`, 우클릭 단일화)와 WORK-0123(로그인 시 자동 실행)을 담았고 WORK-0124로 배포했습니다. 이 기기의 설치 프로그램 실행은 하지 않았습니다.
- **v1.7.0에 포함(WORK-0122)**: 새 프로젝트 화면의 지침·MCP 선택을 **추가 / 추가 안 함** 하나로 통합해 `.claude/CLAUDE.md`·`AGENTS.md`·`.mcp.json`·`.codex/config.toml`을 함께 만들고, Claude 지침 파일을 `.claude/` 안으로 옮겼으며, 우클릭 **지침 쓰기**·**MCP 추가하기**가 Claude·Codex를 한 번에 처리합니다. 상세와 검증은 [[project-organization-and-create-defaults]]와 [[mcp-client-registration]]에 있습니다.
- **v1.7.0에 포함(WORK-0123)**: 로그인 시 자동 실행을 추가했습니다. 배포 빌드 첫 실행에서 켜지고 `--autostart`로 뜬 창은 트레이에만 머물며, 사이드바 아래 **시작 시 자동 실행** 스위치로 끕니다. 동작·기본값·검증은 [[autostart-at-login]]에 있습니다.
...
공개 버전은 1.7.0입니다. 새 프로젝트를 만들 때 지침 파일과 MCP 연결을 "추가 / 추가 안 함" 하나로 고르게 했고, 로그인하면 앱이 트레이에서 스스로 시작되도록 했습니다. 이 기기에 설치 프로그램을 실행하지는 않았습니다.

작업을 시작할 때 진행 중인 기록을 확인하고, 배포·언어 결정·프로젝트 정리·자동 실행 등 주제에 맞는 노트를 읽으시면 됩니다.
